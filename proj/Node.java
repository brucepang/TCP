...
public class Node {
    ...
    private void receivePacket(int from, Packet packet) {
        ...
        case Protocol.TRANSPORT_PKT:
            this.receiveTransport(from,packet);
            break;
    ...
    private void receiveTransport(int srcAddr, Packet packet){
        Transport transportPacket = Transport.unpack(packet.getPayload());
        int destAddr = packet.getDest(); 
        int srcPort = transportPacket.getSrcPort(), destPort = transportPacket.getDestPort();
        // logOutput("Receiving TCP Packet");
        this.tcpMan.receive(srcAddr, srcPort, destAddr, destPort, transportPacket);
    }
